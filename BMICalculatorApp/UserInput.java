...
public class UserInput {
    ...
    public double obtainHeight(String bmiSystem) {
        // Variable to store the heigth value
        double heigth=0;
        // Variable to indicate if the user input is valid or not
        boolean isInvalidInput = true;
        // The loop that controls obtaining user input for weight until a valid value is entered
        while(isInvalidInput){
        Scanner input = new Scanner(System.in);
            // A Switch statement that determines if heigth should be obtained in Pounds or Kilos based on the BMI system
            switch(bmiSystem.toUpperCase()){
                case "IMPERIAL":
                    System.out.println("Enter heigth in Pounds:");
                    break;
                case "METRIC":
                    // Display the message to obtain weight in Kilos
                    System.out.print("Enter heigth in Meter:");
                    // Break out of the switch statement
                    break;
                case "":
                    // Display the message to ask the user to enter a valid value for BMI system
            ...
            // An if statement that checks if the user has entered a valid value (double value) for heigth
            if(input.hasNextDouble()){
